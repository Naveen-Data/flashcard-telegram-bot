...
def get_weak_cards() -> list[dict]:
    """Return cards the user is struggling with — leeches (4+ wrong answers in a row)
    or cards with a low ease_factor. Use this to identify what to re-teach in a study
    session: pull the weak cards, explain the underlying concept again, then let the
    user re-attempt them in the bot.

    Returns:
        List of card dicts (id, question, answer, tags, consecutive_again, ease_factor).
        Empty list if no chat is registered or nothing is currently weak.
    """
    chat_id = db.get_registered_chat_id()
    if chat_id is None:
        return []
